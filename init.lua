...
minetest.register_on_joinplayer(function(obj)
  ...
  minetest.log("action", os.date("Current system time: %H:%M:%S"))
  hour=os.date("%H")
  min=os.date("%M")
  sec=os.date("%S")
  time=(hour+(min+sec/60)/60)/24
  minetest.log("action", "Current game time: "..minetest.get_timeofday())
  minetest.log("action", "Set game time to: "..time)
